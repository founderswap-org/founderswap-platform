...
export default function useUserMedia() {
  ...
  // Video track: combina lo stato videoEnabled con l'Observable della track.
  // Se video è abilitato, ottiene la track dal device,
  // altrimenti restituisce una track "nera" (blackCanvasStreamTrack).
  // Infine, restituisce il track incapsulato in un Observable con "of()"
    ...
    () =>
      combineLatest([
        ...
        switchMap(([track, blur]) =>
          // Se in futuro implementi il blur, potresti chiamare blurVideoTrack(track)
          // per ora restituisce semplicemente il track avvolto in un Observable.
          of(track)
        ),
        shareReplay({
          refCount: true,
